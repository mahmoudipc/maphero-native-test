...
- (void)updateSessionHeaders {
    if (_token) {
        // Update the session configuration with the new headers
        NSMutableDictionary *headers = [NSMutableDictionary dictionary];
        [headers setObject:_token forKey:@"map-token"]; // Add the token as a header

        // Create a new session configuration if it doesn't exist
        if (!_sessionConfig) {
            _sessionConfig = [MLNNetworkConfiguration defaultSessionConfiguration];
        }

        // Add the headers to the session configuration
